...
class ColorJitter:
    ...
    def __call__(self, results):
        ...
        transforms = [(transform_type_dict[k], transform_dict[k]) for k in transform_dict]
        rand_num = np.random.uniform(0, 1, len(transforms))
        for i, (transformer, alpha) in enumerate(transforms):
            r = alpha * (rand_num[i] * 2.0 - 1.0) + 1  # r in [1-alpha, 1+alpha)
            img = transformer(img).enhance(r)
